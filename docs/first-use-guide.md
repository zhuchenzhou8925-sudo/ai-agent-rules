...
本指南面向第一次接触本仓库的用户，帮助你用一个最小案例跑通 Obsidian 项目型知识库整理 Skill。

这不是测试文件、测试脚本、测试用例或自动化验收文件，而是一份上手教程。目标是让你理解：如何让 AI 助手读取 Skill，如何先只读扫描资料，如何确认方案，再生成可放入 Obsidian 的项目笔记、资料索引、MOC 和知识卡片草稿。
...
3. 获得文件清单、分类建议和入库方案。
4. 人工确认处理方案。
5. 生成项目笔记、资料索引、MOC 和知识卡片草稿。
6. 人工复核后，再决定是否迁移到正式 Obsidian 知识库。
...
第一次跑通时，可以先把 AI 输出放在草稿区或单独的临时 Markdown 文件夹中。确认内容正确后，再迁移到正式 Obsidian 知识库。
...
```text
skills/obsidian-project-knowledge-skill/SKILL.md
```
...
```text
请读取 skills/obsidian-project-knowledge-skill/SKILL.md，并按其中的工作流帮我整理一个项目资料文件夹。第一次只做只读扫描，不要移动、删除、重命名或写入正式知识库。
```
...
```text
请只读扫描 example-project-materials/，输出文件清单、分类建议和建议生成的 Obsidian 笔记类型。

要求：
1. 不移动、删除、重命名任何文件。
2. 不写入正式 Obsidian 知识库。
3. 先判断这些资料应如何入库，而不是判断它们有没有价值。
4. 输出项目笔记、资料索引、MOC 和知识卡片的建议。
```
...
- `skills/obsidian-project-knowledge-skill/templates/project-note-template.md`
- `skills/obsidian-project-knowledge-skill/templates/knowledge-card-template.md`
- `skills/obsidian-project-knowledge-skill/templates/moc-template.md`
...
- [Obsidian 项目型知识库整理 Skill](../skills/obsidian-project-knowledge-skill/SKILL.md)
